...
case class OnlineCoursePlatformImpl() extends OnlineCoursePlatform:
  ...
  private var enrollments: Sequence[(student: String, course: String)] = Sequence()
   ...
   * @param course The course to add.
   */
  override def addCourse(course: Course): Unit = this.courses = this.courses.concat(Sequence.Cons(course, Sequence.Nil())).distinct()
   ...
   * @return A sequence of courses in that category.
   */
  override def findCoursesByCategory(category: String): Sequence[Course] = this.courses.filter {
    case CourseImpl(courseId, title, instructor, cat) if category == cat => true
    case _ => false
  }
   ...
   * @return An Optional containing the course if found, otherwise Optional.empty.
   */
  override def getCourse(courseId: String): Optional[Course] = this.courses.find(c => c.courseId == courseId)
   ...
   * @param course The course to remove.
   */
  override def removeCourse(course: Course): Unit = this.courses = this.courses.filter(_ != course)
   ...
   * @return true if the course exists, false otherwise.
   */
  override def isCourseAvailable(courseId: String): Boolean = this.getCourse(courseId) match
   ...
   *                  Fails silently if the course doesn't exist.
   */
  override def enrollStudent(studentId: String, courseId: String): Unit = this.getCourse(courseId) match
    case Optional.Just(course) => this.enrollments = this.enrollments.concat(Sequence.Cons((studentId, course.courseId), Sequence.Nil()))
    case _ => ()
  ...
  override def unenrollStudent(studentId: String, courseId: String): Unit =
    this.enrollments = this.enrollments.filter(_ != (studentId, courseId))
  ...
  override def getStudentEnrollments(studentId: String): Sequence[Course] =
    for
      enrollment <- this.enrollments.filter(enrollment => enrollment.student == studentId).map(e => e.course)
      course <- this.courses if course.courseId == enrollment
    yield course
